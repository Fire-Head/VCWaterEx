...
{
	char			moduleName[MAX_INI];
	char			dllPath[MAX_INI];
	char			iniName[MAX_INI];
	char*			tempPointer;

	GetModuleFileName((HINSTANCE)&__ImageBase, moduleName, MAX_INI);
	tempPointer = strrchr(moduleName, '.');
	*tempPointer = '\0';
	tempPointer = strrchr(moduleName, '\\');
	strncpy(iniName, tempPointer + 1, MAX_INI);
	strcat(iniName, ".ini");
...
void CIniReader::SetIni(char* szFileName)
{
	char			moduleName[MAX_INI];
	char			dllPath[MAX_INI];
	char			iniName[MAX_INI];
...
char* CIniReader::ReadString(char* szSection, char* szKey, const char* szDefaultValue)
{
 ...
 char szResult[1024];
 memset(szResult, 0x00, 1024);
 GetPrivateProfileString(szSection,  szKey, 
		szDefaultValue, szResult, 1024, m_szFileName); 
 return szResult;
}
...
bool CIniReader::SectionExists(char* szSection)
{
	char buff[255] = {0};
	GetPrivateProfileSection(szSection, buff, 255, m_szFileName); 
	return buff[0] != 0;
}

char* CIniReader::GetIniPath()
{
	return m_szFileName;
}

bool CIniReader::IniExists()
{
    DWORD       fileAttr;
    fileAttr = GetFileAttributes(m_szFileName);
