...
RwTexture *&gpWaterEnvBaseTex = *(RwTexture **)0x77FA60;
...
RwRaster *&gpWaterEnvBaseRaster = *(RwRaster **)0x975088;
...
SETTWEAKPATH("PC");
TWEAKFLOAT(fEnvScale,               0.25f, 0.0f,     1.0f,    0.01f);	//	CWaterLevel::CreateWavyAtomic
...
#else
	Int32 slot = CTxdStore::AddTxdSlot("waterfh");
	
	Char Path[MAX_PATH];
	sprintf(Path,"%s\\WaterEx\\FH\\WATER.TXD", GetAsiPath().c_str());
...
#else
	Int32 slot;
	slot = CTxdStore::FindTxdSlot("waterfh");
...
Bool CWaterLevel::GetWaterLevel(float fX, float fY, float fZ, float *pfOutLevel, Bool bDontCheckZ)
{
	...
	*pfOutLevel += fWave * fWindFactor;
	...
	/*
	*pfOutLevel += sinf( ( fTime * (PI / 2048.0f) )
			+ ( ( (Int32(fX) & (WATER_GRID_WIDTH-1)) + (Int32(fY) & (WATER_GRID_WIDTH-1)) ) * (_2PI / WATER_GRID_WIDTH ) ) )
			* ( CWeather::Wind * 0.4f + 0.2f );
	*/
				...
				|| !(aWaterBlockList[2*x+1][2*y+1] & 0x80) )
			{
				...
				if ( fHugeSectorMaxRenderDistSqr > fHugeSectorDistToCamSqr )
				{
							...
							&CMatrix(TheCamera.GetInverseMatrix())) )
					{
						...
						{
#if 0
							//RenderOneSlopedUnderWaterPoly(fX, fY, fZ, colorUnderwater); // just a test
#endif
...
void CWaterLevel::RenderTransparentWater(void)
{
		...
		&& (m_nRenderWaterLayers == 0 || m_nRenderWaterLayers == 2 || m_nRenderWaterLayers == 3) )
	{
		...
		RpMatFXMaterialSetBumpMapFrame(RpGeometryGetMaterial(RpAtomicGetGeometry(ms_pMaskAtomic), 0),
...
void CWaterLevel::PreCalcWaterGeometry(void)
{
	...
	color.green = (Int32)CTimeCycle::m_fCurrentWaterGreen;
	color.blue  = (Int32)CTimeCycle::m_fCurrentWaterBlue;
	color.alpha = (Int32)CTimeCycle::m_fCurrentWaterAlpha;
	...
	CVector upCamDir = TheCamera.m_sCoords.up;
	upCamDir.z = 0.0f;
	upCamDir.Normalise();
	
	Float fCamX = TheCamera.m_sCoords.pos.x - (Float)SMALL_SECTOR_SIZE;
	Float fCamY = TheCamera.m_sCoords.pos.y - (Float)SMALL_SECTOR_SIZE;
	
	//1.4144272f; 1.4144f;
	Float signX = upCamDir.x * 1.4144272f;
	Float signY = upCamDir.y * 1.4144272f;
	...
	if ( _IsColideWithBlock(BlockX, BlockY, nBlock) )
	{
		...
		if ( PreCalcWavyMask( fMaskX, fMaskY, ms_aWaterZs[nBlock],
				fSectorX, fSectorY, fCamX, fCamY, upCamDir.x, upCamDir.y, color ) )
        {
...
{	
	...
	CVector camPosUp = TheCamera.m_sCoords.up;
	...
	Float waveA = (_2PI / 16.0f)
		* ((CWeather::WindClipped * 0.4f + 0.2f) * (fNormalDirectionScalar1 * fabs(camPosUp.x + camPosUp.y) + fNormMult));
		
	Float waveB = _2PI / (16.0f * fWave2NormScale)
		* ((CWeather::WindClipped * 0.2f + 0.1f) * (fNormalDirectionScalar2 * fabs(camPosUp.y - camPosUp.x) + fNormMultB));
		...
		float fDrawDist)
{
	...
	Float fDistStartX = (fX                       - fCamX) * (fX                       - fCamX);
	Float fDistStartY = (fY                       - fCamY) * (fY                       - fCamY);
	Float fDistEndX   = ((fX + SMALL_SECTOR_SIZE) - fCamX) * ((fX + SMALL_SECTOR_SIZE) - fCamX);
	Float fDistEndY   = ((fY + SMALL_SECTOR_SIZE) - fCamY) * ((fY + SMALL_SECTOR_SIZE) - fCamY);
